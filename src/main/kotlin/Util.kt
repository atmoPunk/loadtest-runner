...
/** Code grabbed from https://github.com/JetBrains/teamcity-google-agent/blob/master/google-cloud-server/src/main/kotlin/jetbrains/buildServer/clouds/google/connector/ApiFuture.kt */
...
suspend fun <T: Closeable> Collection<T>.useAll(block: suspend (Collection<T>) -> Unit) {
    try {
        block(this)
    } finally {
        this.forEach { it ->
...
suspend fun <T> retry(
    times: Int = 3,
    onException: (Int, Exception) -> Unit,
    block: suspend () -> T
): T {
    repeat(times - 1) { it ->
        try {
